...
export function createPageRange(currentPage: number, numPages: number, pageRange: number): Array<string | number> {
  ...
  if (newPageCount <= availableSlots && oldPageCount <= availableSlots) {
    ...
    return [...range(1, currentPage + availableSlots - 2 + availableSlots - newPageCount + 1), '...', numPages];
  } else if (newPageCount > availableSlots && oldPageCount <= availableSlots) {
    // [1, ... , currentPage - (ava - 2) - (ava - oldPageCount)) => (numPages)]
    return [1, '...', ...range(currentPage - (availableSlots - 2) - (availableSlots - oldPageCount), numPages + 1)];
  } else {
    ...
    return [
      1,
      '...',
      ...range(currentPage - (availableSlots - 2), currentPage + (availableSlots - 2) + 1),
      '...',
      numPages,
    ];
  }
